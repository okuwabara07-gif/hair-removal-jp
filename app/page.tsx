export default function Home() {
  return (
    <main style={{fontFamily:'sans-serif',maxWidth:'900px',margin:'0 auto',padding:'2rem'}}>
      <h1 style={{fontSize:'1.5rem',fontWeight:700,marginBottom:'1rem'}}>脱毛・ムダ毛ケア情報</h1>
      <p style={{color:'#666',marginBottom:'2rem'}}>最新の脱毛情報をご覧ください。</p>

      <div style={{margin:'2rem 0',padding:'1.5rem',background:'linear-gradient(135deg,#fdf4ff,#fae8ff)',borderRadius:'16px',border:'2px solid #c084fc'}}>
        <p style={{fontSize:'0.7rem',color:'#7e22ce',fontWeight:700,marginBottom:'0.75rem'}}>🏆 おすすめ脱毛サービス</p>
        <a href="https://px.a8.net/svt/ejp?a8mat=4AZR8U+GB1F8Y+54TM+HV7V6" target="_blank" rel="noopener noreferrer sponsored"
          style={{display:'flex',alignItems:'center',gap:'12px',padding:'12px 16px',background:'white',borderRadius:'12px',textDecoration:'none',border:'1px solid #c084fc'}}>
          <span style={{fontSize:'1.5rem'}}>✨</span>
          <div>
            <div style={{fontSize:'0.85rem',fontWeight:700,color:'#333'}}>トリア - プロが選ぶ家庭用脱毛器（購入5%）</div>
            <div style={{fontSize:'0.7rem',color:'#7e22ce'}}>サロン級の効果を自宅で・確定率73%</div>
          </div>
